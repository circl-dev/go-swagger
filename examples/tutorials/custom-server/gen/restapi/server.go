...
// NewServer creates a new api greeter server but does not configure it
func NewServer(api *operations.GreeterAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/greeter.sock"`
...
func (s *Server) SetAPI(api *operations.GreeterAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
