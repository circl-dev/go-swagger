...
go-swagger releases are distributed as binaries that are built from signed tags. It is published [as github release](https://github.com/circl-dev/go-swagger/tags),
...
<https://github.com/circl-dev/go-swagger/releases/latest>

```
download_url=$(curl -s https://api.github.com/repos/circl-dev/go-swagger/releases/latest | \
...
```
dir=$(mktemp -d) 
git clone https://github.com/circl-dev/go-swagger "$dir" 
cd "$dir"
...
go install -ldflags "-X github.com/circl-dev/go-swagger/cmd/swagger/commands.Version=$(git describe --tags) -X github.com/circl-dev/go-swagger/cmd/swagger/commands.Commit=$(git rev-parse HEAD)" ./cmd/swagger
```
