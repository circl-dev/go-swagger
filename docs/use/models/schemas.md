...
- a validation interface ([`circl-dev/runtime/Validatable`][Validatable]), with a `Validate(strfmt.Registry) error` method
...
> the [`circl-dev/validate`][validate] package
...
- re [JSON-schema-draft4][json-schema]
  ...
  - when `enum` values cannot be marshalled into their schema, a runtime panic occurs - the `circl-dev/validate` package does not yet detect this situation
  - `patternProperties` and `dependencies`are not supported
  - use of `additionalItems` requires the `--skip-validation` flag (`circl-dev/validate` is strict regarding Swagger specification)
...
runtime server or client is provided by the [`circl-dev/runtime/File` type][File].
...
> feel free to comment and express your views [here](https://github.com/circl-dev/go-swagger/issues/232).
...
Note that the external model must implement the `github.com/circl-dev/runtime.Validatable` interface: it must know how to validate a schema.
...
definitions:
  ...
  Hotspot:
    x-go-type:
      type: Hotspot
      import:
        package: github.com/circl-dev/go-swagger/fixtures/enhancements/2224/external
      hints:
        kind: object
    x-nullable: true
```
...
[runtime]: https://github.com/circl-dev/runtime
[File]: https://github.com/circl-dev/runtime/blob/master/file.go#L20
[Validatable]: https://github.com/circl-dev/runtime/blob/master/interfaces.go#L101
[validate]: https://github.com/circl-dev/validate
[validate-json]: https://godoc.org/github.com/circl-dev/validate#ex-AgainstSchema
[go-doc-model]: https://godoc.org/github.com/circl-dev/go-swagger/examples/generated/models
...
[lifting-pointers]: https://github.com/circl-dev/go-swagger/pull/557
